


export const navData=[
    {
        link:'Home',
        path:'/',
    },
    {
        link:'Pages',
        path:'/pages',
    },
    {
        link:'User Account',
        path:'/user-account',
    },
    {
        link:'Vender Account',
        path:'/vender-account',
    },
    {
        link:'Contact',
        path:'/contact',
    },
]
export const categoryData=[
    {
        name:'Fashion',
        img:"./images/category/cat1.png",
    },
    {
        name:'Electronic',
        img:"./images/category/cat2.png",
    },
    {
        name:'Cars',
        img:"./images/category/cat3.png",
    },
    {
        name:'Home & Garden',
        img:"./images/category/cat4.png",
    },
    {
        name:'Gifts',
        img:"./images/category/cat5.png",
    },
    {
        name:'Music',
        img:"./images/category/cat6.png",
    },
    {
        name:'Health & Beauty',
        img:"./images/category/cat7.png",
    },
    {
        name:'Pets',
        img:"./images/category/cat8.png",
    },
    {
        name:'Baby Toys',
        img:"./images/category/cat9.png",
    },
    {
        name:'Groceries',
        img:"./images/category/cat10.png",
    },
    {
        name:'Books',
        img:"./images/category/cat11.png",
    },
]
export const dealsData=[
    {
        name:'Shoes',
        icon:<i className='fa fa-star'></i>,
        price:100.00,
        image:'./images/flash/flash-1.png'
    },
    {
        name:'Watch',
        icon:<i className='fa fa-star'></i>,
        price:20.00,
        image:'./images/flash/flash-2.png'
    },
    {
        name:'Smart Mobile Black',
        icon:<i className='fa fa-star'></i>,
        price:200.00,
        image:'./images/flash/flash-3.png'
    },
    {
        name:'Smart Watch Black',
        icon:<i className='fa fa-star'></i>,
        price:50.00,
        image:'./images/flash/flash-4.png'
    },
]
export const categoryCoverData=[
    {
        name:'sunglass',
        img:'./images/top/category-3.png',
        desc:'6k orders this week',
    },
    {
        name:'headphone',
        img:'./images/top/category-1.png',
        desc:'3k orders this week',
    },
    {
        name:'watches',
        img:'./images/top/category-2.png',
        desc:'4k orders this week',
    },
]
export const discountData=[
    {
        name:'Sony PS4',
        price:400,
        img:'./images/discount/discount-3.png'
    },
    {
        name:'Benux 2022',
        price:200,
        img:'./images/discount/discount-1.png'
    },
    {
        name:'Setgear 2022',
        price:100,
        img:'./images/discount/discount-4.png'
    },
    {
        name:'Tony BGB',
        price:50,
        img:'./images/discount/discount-5.png'
    },
    {
        name:'RG Products',
        price:300,
        img:'./images/discount/discount-6.png'
    },
    {
        name:'Sony TV 1080p',
        price:540,
        img:'./images/discount/discount-2.png'
    },
]

export const shopsData=[
    {
        name:'Apple',
        img:'./images/category/cat-1.png',
    },
    
    {
        name:'Samasung',
        img:'./images/category/cat-2.png',
    },
    
    {
        name:'Oppo',
        img:'./images/category/cat-1.png',
    },
    
    {
        name:'Vivo',
        img:'./images/category/cat-2.png',
    },
    
    {
        name:'Redimi',
        img:'./images/category/cat-1.png',
    },
    
    {
        name:'Sony',
        img:'./images/category/cat-2.png',
    },

    {
        name:'Hawawii',
        img:'./images/category/cat-1.png',
    },
]
export const shopItemsData=[
    {
        name:'Mapple Earphones',
        brand:'25% off',
        price:180.00,
        img:'./images/shops/shops-1.png'
    },
    {
        name:'Vivo android one',
        brand:'10% off',
        price:120.00,
        img:'./images/shops/shops-2.png'
    },
    {
        name:'Sony Light',
        brand:'50% off',
        price:20.00,
        img:'./images/shops/shops-3.png'
    },
    {
        name:'Iphone',
        brand:'10% off',
        price:920.00,
        img:'./images/shops/shops-4.png'
    },
    {
        name:'Ceats wireless earphone',
        brand:'25% off',
        price:80.00,
        img:'./images/shops/shops-5.png'
    },
    {
        name:'Redimi Phone',
        brand:'20% off',
        price:400.00,
        img:'./images/shops/shops-6.png'
    },
    {
        name:'Xeats Bluetooth earphones',
        brand:'15% off',
        price:60.00,
        img:'./images/shops/shops-7.png'
    },
    {
        name:'Airpod',
        brand:'10% off',
        price:120.00,
        img:'./images/shops/shops-8.png'
    },
    {
        name:'Sliver Cap',
        brand:'5% off',
        price:10.00,
        img:'./images/shops/shops-9.png'
    },
]

export const ServicesData=[
    {
        cover: <i class='fa-solid fa-truck-fast'></i>,
        title: "Worldwide Delivery",
        decs: "We offer competitive prices on our 100 million plus product any range.",
      },
      {
        cover: <i class='fa-solid fa-id-card'></i>,
        title: "Safe Payment",
        decs: "We offer competitive prices on our 100 million plus product any range.",
      },
      {
        cover: <i class='fa-solid fa-shield'></i>,
        title: "Shop With Confidence ",
        decs: "We offer competitive prices on our 100 million plus product any range.",
      },
      {
        cover: <i class='fa-solid fa-headset'></i>,
        title: "24/7 Support ",
        decs: "We offer competitive prices on our 100 million plus product any range.",
      },
]

export const aboutUs=[
    {Link:'Careers'},
    {Link:'Our Stores'},
    {Link:'Our Cares'},
    {Link:'Terms & Conditions'},
    {Link:'Privacy Policy'},
]

export const Customer=[
    {Link:'Help Center'},
    {Link:'How to Buy '},
    {Link:'Track Your Order'},
    {Link:'Corporate & Bulk Purchasing'},
    {Link:'Returns & Refunds '},
    
]

export const ContactUs=[
    {Link:'70 Washington Square South, New York, NY 10012, United States '},
    {Link:'Email: hussain@gmail.com'},
    {Link:'Phone: +964 1123 456 780'},
    
]